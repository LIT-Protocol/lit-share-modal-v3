...
const DevModeHeader = ({handleClose, isModal, showDevMode, setShowDevMode}) => {
  return (
    <header className={'lsm-bg-brand-light lsm-header-container'}>
      <h3 className={'lsm-text-brand-6 lsm-header-text lsm-font-light lsm-font-segoe'}>Dev Mode</h3>
      <button onClick={() => setShowDevMode(!showDevMode)}>{showDevMode ? 'Hide Dev' : 'Show Dev'}</button>
      {isModal && (
